...
import { Calendar, Clock, MapPin, Video, Users, Mail, Phone, User, Building, Image, CheckCircle, XCircle } from 'lucide-react';
...
export default function Workshop() {
    ...
    const inputClassName = "w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#00565c]/20 focus:border-[#00565c] transition-all duration-200 bg-white";
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
                ...
                <div className="mb-8">
                    <div className="flex items-center justify-between">
                        {['Basic Info', 'Details', 'Contact'].map((step, index) => (
                            <div key={step} className="flex items-center flex-1">
                                <div className="relative">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold
                    ${currentStep > index + 1 ? 'bg-[#00565c] text-white' :
                                            currentStep === index + 1 ? 'bg-[#00565c] text-white ring-4 ring-[#00565c]/20' :
                                                'bg-gray-200 text-gray-600'}`}>
                                        {currentStep > index + 1 ? '✓' : index + 1}
                                    </div>
                                </div>
                                <div className={`ml-3 flex-1 ${index < 2 ? 'mr-4' : ''}`}>
                                    <p className={`text-sm font-medium ${currentStep >= index + 1 ? 'text-[#00565c]' : 'text-gray-500'}`}>
                                        {step}
                                    </p>
                                </div>
                                {index < 2 && (
                                    <div className={`flex-1 h-0.5 ${currentStep > index + 1 ? 'bg-[#00565c]' : 'bg-gray-200'}`} />
                                )}
                            </div>
                        ))}
                    </div>
                ...
                <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="md:col-span-2">
                                    <label className={labelClassName}>Workshop Title *</label>
                                    <input
                                        name="title"
                                        value={form.title}
                    ...
                    {currentStep === 2 && (
                        <div className="space-y-6">
                            <h2 className="text-xl font-semibold text-gray-800 mb-4">Schedule & Venue</h2>
                                ...
                                <div className="md:col-span-2">
                                    <label className="flex items-center gap-3 cursor-pointer">
                                        <input
                                            ...
                                            onChange={handleChange}
                                            className="w-5 h-5 text-[#00565c] border-gray-300 rounded focus:ring-[#00565c]"
                                        />
                    ...
                    <div className="flex justify-between mt-8 pt-6 border-t border-gray-200">
                        ...
                        {currentStep < 3 ? (
                            <button
                                type="button"
                                onClick={nextStep}
                                className="ml-auto px-6 py-2.5 bg-[#00565c] text-white rounded-lg hover:bg-[#004348] transition-colors font-medium"
                            >
                                Next
                            </button>
                        ) : (
                            <button
                                type="submit"
                                disabled={loading}
                                className="ml-auto px-8 py-2.5 bg-[#00565c] text-white rounded-lg hover:bg-[#004348] transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                            >
                                {loading ? (
                                    <>
                        ...
                        <div className="border border-gray-200 rounded-xl overflow-hidden">
                            ...
                            <div className="p-6">
                                <div className="flex items-center gap-2 mb-2">
                                    <span className="px-3 py-1 bg-[#00565c]/10 text-[#00565c] text-sm font-medium rounded-full">
                                        {form.category}
                                    </span>
            ...
            <style>{`
        .input {
          @apply w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00565c] focus:border-transparent;
        }
      `}</style>
        </div>
    );
